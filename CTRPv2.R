...
curationTissue <- data.frame(unique.tissueid = cell_all[curationCell$unique.cellid, "unique.tissueid"], "CTRPv2.tissueid" = ctrp.cells$tissueid[which(rownames(ctrp.cells) %in% curationCell$unique.cellid)])
