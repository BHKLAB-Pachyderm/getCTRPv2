...
colnames(ctrp.drugs)[which(names(ctrp.drugs) == "drugid")] <- "unique.drugid"  
...
colnames(ctrp.cells)[which(names(ctrp.cells) == "cellid")] <- "unique.cellid"
disease <- cell_all$Cellosaurus.Disease.Type[match(ctrp.cells$unique.cellid, cell_all$unique.cellid)]
ctrp.cells$Cellosaurus.Disease.Type <- disease		 
	
#add cellosaurus assession to cell-info
assession <- cell_all$Cellosaurus.Accession.id[match(ctrp.cells$unique.cellid, cell_all$unique.cellid)]
ctrp.cells$Cellosaurus.Accession.id <- assession
		 
#add pharmacodb id to cell-info
pdb <- cell_all$PharmacoDB.id[match(ctrp.cells$unique.cellid, cell_all$unique.cellid)]
ctrp.cells$PharmacoDB.id <- pdb

#add study tissue id to cell_info
study_tissue <- cell_all$unique.tissueid.fromstudies[match(ctrp.cells$unique.cellid, cell_all$unique.cellid)]
ctrp.cells$unique.tissueid.fromstudies <- study_tissue
		 
#add study cell-line type to cell_info
cell_type <- cell_all$CellLine.Type[match(ctrp.cells$unique.cellid, cell_all$unique.cellid)]
ctrp.cells$CellLine.Type <- cell_type
		 
#add metastatic info to cell_info		 
metastatic <- cell_all$Metastatic[match(ctrp.cells$unique.cellid, cell_all$unique.cellid)]
